...
3. 手动运行 `Update NetEase listening stats`，确认生成的 `netease-stats.json` 中 `duration.source` 为 `netease-listen-data` 且 `duration.available` 为 `true`。如果网易云接口暂时不可用（例如 Cookie 过期或账号权限限制），页面会显示 `—`，不会用 Top 20 播放次数估算冒充总时长；Actions 日志只出现摘要，不会打印 Cookie。
